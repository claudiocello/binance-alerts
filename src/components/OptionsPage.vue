    ...
    <div class="push-bottom">
      ...
      <Tabs class="push-bottom">
        ...
        <section btn-class="icon-network" btn-name="CORS Proxy" active>
          <form
            class="cors-form flex-row flex-middle flex-stretch push-bottom"
            action="#"
          ...
          >
            <div class="form-input flex-1 push-right">
              ...
              <span
                class="push-left"
                :class="{
                  'icon-check text-gain': urlSuccess,
                  'icon-close text-loss': !urlSuccess,
                  'icon-clock text-warning': testing,
                }"
              ></span>
            </div>
            <button
              class="form-btn bg-info-hover push-right push-small"
              type="submit"
            >
          ...
          <div class="flex-list push-bottom">
            <div
              ...
              :key="p.url"
              class="flex-item flex-row flex-middle flex-stretch"
            >
              <div
                class="flex-1 icon-network iconLeft clickable text-clip push-right"
              ...
              <button
                class="icon-close text-danger-hover"
                type="button"
          ...
          <div class="text-small text-grey">
            <p>
              External proxy server used to route outgoing HTTP requests from
              this app to get around the browser's built-in
              <a
                class="text-nowrap"
        ...
        <section btn-class="icon-chart-line" btn-name="Binance API">
          ...
          <div class="form-input push-bottom">
            <span class="text-nowrap text-grey push-right"
              >Binance API Key:</span
            >
            <input
              class="flex-1"
              type="text"
              placeholder="..."
              v-model="options.binance.apikey"
              @blur="saveOptions()"
            />
          </div>
          <div class="form-input push-bottom">
            <span class="text-nowrap text-grey push-right"
              >Binance API Secret:</span
            >
            <input
              class="flex-1"
              type="text"
              placeholder="..."
              v-model="options.binance.apisecret"
              @blur="saveOptions()"
            />
          </div>
          <div class="text-small text-grey">
            <p>
              Provide your
              <a href="#" @click.prevent="goBinance">Binance API keys</a>
              if you wish to use the trading features available within this app.
              The key you enter here will be stored in your web browser's local
              storage for use within this app only. You can find your key, or
              create a new one by accessing your account page on the Binance
              website.
            </p>
          </div>
        </section>
          ...
          <div class="text-small text-grey">
            <p>
              ...
              <a
                ...
                >Mailgun API</a
              >
              for sending outgoing e-mail notifications using a Mailgun account.
              You will need to add your Mailgun account details above.
            </p>
          </div>
        </section>
          ...
          <div class="text-small text-grey">
            <p>
              ...
              <a
                ...
                >Telegram Bot API</a
              >
              for sending outgoing notifications using the Telegram app on
              desktop or mobile. You will need to provide a Bot API Token and
              the User chat ID above. You can use a bot such as
...
export default {
  ...
  methods: {
    ...
    // apply options
    async saveOptions(audio) {
      try {
        let options = Object.assign({}, this.options, {
          proxy: this.corsProxy,
        });
        if (this.options.telegram.enabled) {
          const botToken = this.options.telegram.botkey;
          const chatId = this.options.telegram.userid;

          await fetch(
            "https://binance-alerts-backend-production.up.railway.app/save-telegram-config",
            {
              method: "POST",
              headers: {
    ...
    testProxy(url) {
      ...
      this.$ajax.get(url, {
        ...
        success: (xhr, status, response) => {
          ...
          this.$bus.emit(
            "showNotice",
            "Proxy URL responded with status code (" + status + ").",
            clss
          );
        },
      });
